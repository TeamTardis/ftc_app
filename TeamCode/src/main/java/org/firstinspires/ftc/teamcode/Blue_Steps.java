...
public class Blue_Steps extends TardisOpModeAutonomous { //Imports presets for initiation from TardisOpModeAutonomus
    ...
    public enum steps { //All steps for completing autonomous
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////
        //Contingency steps - These steps run in case there is an unexpected sensor reading or none at all.//
        ////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public void loop() { //Starts loop for the program
        ...
        switch (CURRENT_STEP) { //Beginning of the switch- this sets the current step to whatever CURRENT_STEP is set to
            ...
            case ROTATE_TO_VORTEX: //Beginning of case statement ROTATE_TO_VORTEX
                ...
                if (gyro.getIntegratedZValue() > 18) { //Rotates robot about 20° clockwise so the launcher faces the center vortex
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
            ...
            case ROTATE_BACK: //Beginning of case statement ROTATE_BACK
                ...
                if (gyro.getIntegratedZValue() < -75) { //Rotate counter-clockwise so beacon pusher faces toward beacon wall and turn off launcher motors
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
                ...
                m1.setPower(.15); //Sets motor 1 to power -.1 to rotate the robot counter-clockwise
                m2.setPower(-.15); //Sets motor 2 to power .1 to rotate the robot counter-clockwise
                m3.setPower(.15); //Sets motor 3 to power -.1 to rotate the robot counter-clockwise
                m4.setPower(-.15); //Sets motor 4 to power .1 to rotate the robot counter-clockwise
            ...
            case DRIVE_TO_FIRST_BEACON: //Beginning of case statement DRIVE_TO_FIRST_BEACON
                ...
                if (gyro.getIntegratedZValue() < -90) { //If gyro senses a tilt, it lowers the speed of motor 1 to correct itself
                    ...
                    m2.setPower(1); //Sets motor 2 to power 0 to go diagonally toward beacon one
                    m3.setPower(.8); //Sets motor 3 to power 0 to go diagonally toward beacon one
                ...
                if (gyro.getIntegratedZValue() > -90) { //If gyro senses a tilt, it lowers the speed of motor 4 to correct itself
                    ...
                    m2.setPower(.8); //Sets motor 2 to power 0 to go diagonally toward beacon one
                    m3.setPower(1); //Sets motor 3 to power 0 to go diagonally toward beacon one
                    m4.setPower(0); //Sets motor 4 to power .8 to go diagonally toward beacon one
                    break; //Exits switch statement
                } //End of if statement
                if (gyro.getIntegratedZValue() == -90) { //If gyro senses no tilt, it will continue to move at full power with both motors 1 and 4
                    m1.setPower(0); //Sets motor 1 to power 1 to go diagonally toward beacon one
                    m2.setPower(1); //Sets motor 2 to power 0 to go diagonally toward beacon one
                    m3.setPower(1); //Sets motor 3 to power 0 to go diagonally toward beacon one
                    m4.setPower(0); //Sets motor 4 to power 1 to go diagonally toward beacon one
  ...
  /**              /////////////////////////////////////////////
            ...
            case FIND_WHITE_LINE_BEACON_ONE: //Beginning of case statement FIND_WHITE_LINE_BEACON_ONE
                ...
                if (odsSensor1.getRawLightDetected() > .5 || runtime.seconds() > 0.5) { //Moves right in order to prepare for scan of colors for .8 seconds
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
...
**/
            ...
            case FIND_CORRECT_COLOR_BEACON_ONE: //Beginning of case statement FIND_CORRECT_COLOR_BEACON_ONE
                ...
                if (colorSensor.blue() > 150) { //Moves right until red is found
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
            ...
            case NEXT_BEACON_STEP_ONE: //Beginning of case statement NEXT_BEACON_STEP_ONE
                ...
                if (runtime.seconds() > 1) { //Moves right for .8 seconds to get ODS off beacon one white line
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
                ...
                if (gyro.getIntegratedZValue() < -90) { //If gyro senses a tilt, it lowers the speed of motor 1 and motor 2 to correct itself
                    m1.setPower(-.7); //Sets motor 1 to power -.3 to go left to scan for the correct color
                    m2.setPower(.7); //Sets motor 2 to power .3 to go left to scan for the correct color
                    m3.setPower(.5); //Sets motor 3 to power .4 to go left to scan for the correct color
                    m4.setPower(-.5); //Sets motor 4 to power -.4 to go left to scan for the correct color
                    break; //Exits switch statement
                } //End of if statement
                if (gyro.getIntegratedZValue() > -90) { //If gyro senses a tilt, it lowers the speed of motor 3 and motor 4 to correct itself
                    m1.setPower(-.5); //Sets motor 1 to power -.4 to go left to scan for the correct color
                    m2.setPower(.5); //Sets motor 2 to power .4 to go left to scan for the correct color
                    m3.setPower(.7); //Sets motor 3 to power .3 to go left to scan for the correct color
                    m4.setPower(-.7); //Sets motor 4 to power -.3 to go left to scan for the correct color
                    break; //Exits switch statement
                } //End of if statement
                if (gyro.getIntegratedZValue() == -90) { //If gyro senses no tilt, it continues to go right with all drive train motors set to a power of .3
                    m1.setPower(-.7); //Sets motor 1 to power -.4 to go left to scan for the correct color
                    m2.setPower(.7); //Sets motor 2 to power .4 to go left to scan for the correct color
                    m3.setPower(.7); //Sets motor 3 to power .4 to go left to scan for the correct color
                    m4.setPower(-.7); //Sets motor 4 to power -.4 to go left to scan for the correct color
                    break; //Exits switch statement
                } //End of if statement
                break;
            ...
            case NEXT_BEACON_STEP_TWO: //Beginning of case statement NEXT_BEACON_STEP_TWO
                ...
                else if (range.getDistance(DistanceUnit.CM) < 10 || range.getDistance(DistanceUnit.CM) > 30) {
            ...
            case RANGE_READJUST_BEACON_ONE:
                ...
                if (range.getDistance(DistanceUnit.CM) < 14) {
                    if (colorSensor.blue() > 150) { //Moves right until next white line is found
                        m1.setPower(0); //Sets motor 1 to power 0 before next step
                        m2.setPower(0); //Sets motor 2 to power 0 before next step
                        m3.setPower(0); //Sets motor 3 to power 0 before next step
                        m4.setPower(0); //Sets motor 4 to power 0 before next step
                    ...
                    m3.setPower(0); //Sets motor 3 to power .2 to go left to scan for the correct color
                    m4.setPower(-.3); //Sets motor 4 to power -.2 to go left to scan for the correct color
                } else if (range.getDistance(DistanceUnit.CM) > 14) {
                    if (colorSensor.blue() > 150) { //Moves right until next white line is found
                        m1.setPower(0); //Sets motor 1 to power 0 before next step
                        m2.setPower(0); //Sets motor 2 to power 0 before next step
                        m3.setPower(0); //Sets motor 3 to power 0 before next step
