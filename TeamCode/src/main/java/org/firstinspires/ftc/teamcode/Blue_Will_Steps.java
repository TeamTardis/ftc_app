...
public class Blue_Will_Steps extends TardisOpModeAutonomous { //Imports presets for initiation from TardisOpModeAutonomus
    ...
    public enum steps { //All steps for completing autonomus
        ...
        FIND_CORRECT_COLOR_BEACON_ONE, //[Step 12] Drive left to locate and stop at correct color while scanning beacon
        ...
        NEXT_BEACON_STEP_ONE, //[Step 15] Moves left to get off white line to prepare to look for the beacon two white line
        NEXT_BEACON_STEP_TWO, //[Step 16] Using gyro for stablization, begin to start scan for the beacon two white line
        DRIVE_OFF_WHITE_LINE_BEACON_TWO, //[Step 17] Move off white line to right side of the beacon to prepare to scan for correct color
        FIND_CORRECT_COLOR_BEACON_TWO, //[Step 18] Drive left to locate and stop at correct color while scanning beacon
        ...
        HIT_CAP_BALL, //[Step 21] Drive diagonally backward using motor 2, motor 3, and gyro sensor to hit cap ball off the base of the center vortex and park there
    ...
    private ElapsedTime runtime = new ElapsedTime(); //Creates a varible for runtime so the robot can preform timed actions
    ...
    public void loop() { //Starts loop for the program
        ...
        switch (CURRENT_STEP) { //Beginning of the switch- this sets the current step to whatever the CURRENT_STEP varible is set to
            ...
            case ROTATE_TO_VORTEX:
                ...
                if (gyro.getHeading() > 325 && gyro.getHeading() < 335) { //Rotates robot about 50° counter-clockwise so the launcher faces the center vortex
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
            ...
            case LOWER_SERVO_2:
                ...
                if (runtime.seconds() > .3) { //Lowers launching servo to original position, set the launcher motors' power to -.1 and .1 and stops the sweeperOn for 1 second
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
                ...
                m5.setPower(.1); //Sets laucher motor's power to -.1 to slow it down
            ...
            case ROTATE_BACK:
                ...
                if (gyro.getHeading() > 86 && gyro.getHeading() < 94) { //Rotate clockwise to face starting direction- beacon pusher toward beacons and turn off launcher motors
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
            ...
            case STRAIGHTEN_ON_WALL:
                ...
                m1.setPower(.3); //Sets motor 1 to power .3 to go right to straighten against wall
                m2.setPower(-.3); //Sets motor 2 to power -.3 to go right to straighten against wall
                m3.setPower(-.3); //Sets motor 3 to power -.3 to go right to straighten against wall
                m4.setPower(.3); //Sets motor 4 to power .3 to go right to straighten against wall
                break; //Exits switch statement
            ...
            case DRIVE_TO_FIRST_BEACON:

                if (odsSensor1.getRawLightDetected() > .4) { //Moves diagonally towards first beacon and stops when ODS sences a white line
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
                ...
                if (gyro.getHeading() < 270) { //If gyro senses a tilt, it lowers the speed of motor 3 to correct itself < 180
                    m1.setPower(0); //Sets motor 1 to power 0 to go diagonally toward beacon one
                    m2.setPower(1); //Sets motor 2 to power 1 to go diagonally toward beacon one
                    m3.setPower(.8); //Sets motor 3 to power .8 to go diagonally toward beacon one
                    m4.setPower(0); //Sets motor 4 to power 0 to go diagonally toward beacon one
                    break; //Exits switch statement
                } //End of if statement
                if (gyro.getHeading() < 90) { //If gyro senses a tilt, it lowers the speed of motor 2 to correct itself
                    m1.setPower(0); //Sets motor 1 to power 0 to go diagonally toward beacon one
                    m2.setPower(.8); //Sets motor 2 to power .8 to go diagonally toward beacon one
                    m3.setPower(1); //Sets motor 3 to power 1 to go diagonally toward beacon one
                    m4.setPower(0); //Sets motor 4 to power 0 to go diagonally toward beacon one
                    break; //Exits switch statement
                } //End of if statement
                if (gyro.getHeading() == 90) { //If gyro senses no tilt, it will continue to move at full power with both motor 2 and 3
                    m1.setPower(0); //Sets motor 1 to power 0 to go diagonally toward beacon one
                    m2.setPower(1); //Sets motor 2 to power 1 to go diagonally toward beacon one
                    m3.setPower(1); //Sets motor 3 to power 1 to go diagonally toward beacon one
                    m4.setPower(0); //Sets motor 4 to power 0 to go diagonally toward beacon one
            ...
            case DRIVE_OFF_WHITE_LINE_BEACON_ONE:
                ...
                if (runtime.seconds() > 1) { //Moves right in order to prepare for scan of colors for .8 seconds
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
                ...
                if (gyro.getHeading() < 90) { //If gyro senses a tilt, it lowers the speed of motor 3 and motor 4 to correct itself
                    m1.setPower(.3); //Sets motor 1 to power .2 to go left to scan for the correct color
                    m2.setPower(-.3); //Sets motor 2 to power -.2 to go left to scan for the correct color
                    m3.setPower(-.2); //Sets motor 3 to power -.1 to go left to scan for the correct color
                    m4.setPower(.2); //Sets motor 4 to power .1 to go left to scan for the correct color
                    break; //Exits switch statement
                } //End of if statement
                if (gyro.getHeading() > 90) { //If gyro senses a tilt, it lowers the speed of motor 1 and motor 2 to correct itself
                    m1.setPower(.2); //Sets motor 1 to power -.1 to go left to scan for the correct color
                    m2.setPower(-.2); //Sets motor 2 to power .1 to go left to scan for the correct color
                ...
                if (gyro.getHeading() == 90) { //If gyro senses no tilt, it continues to go right with all drive train motors set to a power of .3
                    m1.setPower(.2); //Sets motor 1 to power -.2 to go left to scan for the correct color
                    m2.setPower(-.2); //Sets motor 2 to power .2 to go left to scan for the correct color
                    m3.setPower(-.2); //Sets motor 3 to power .2 to go left to scan for the correct color
                    m4.setPower(.2); //Sets motor 4 to power -.2 to go left to scan for the correct color
                    break; //Exits switch statement
                } //End of if statement
                break; //Exits switch statement
            ...
            case FIND_CORRECT_COLOR_BEACON_ONE:
                ...
                if (colorSensor.blue() > 1.5) { //Moves left until blue is found
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
                    ...
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
                    runtime.reset(); //Resets time before switching to next step
                    CURRENT_STEP = steps.NEXT_BEACON_STEP_TWO; //Sets next step a backup step- NEXT_BEACON_STEP_TWO
                    break; //Exits switch statement
            ...
            case PUSH_BUTTON_BEACON_ONE:
                ...
                m1.setPower(.4); //Sets motor 1 to power .2 to go forward to push button
                m2.setPower(.4); //Sets motor 2 to power .2 to go forward to push button
                m3.setPower(.4); //Sets motor 3 to power .2 to go forward to push button
                m4.setPower(.4); //Sets motor 4 to power .2 to go forward to push button
                break; //Exits switch statement
            ...
            case NEXT_BEACON_STEP_ONE:
                ...
                if (runtime.seconds() > 1) { //Moves left for 1 second to get ODS off beacon one white line
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
                ...
                m1.setPower(-.7); //Sets motor 1 to power -.2 to move left so OBS gets off beacon one white line
                m2.setPower(.7); //Sets motor 2 to power .2 to move left so OBS gets off beacon one white line
                m3.setPower(.7); //Sets motor 3 to power .2 to move left so OBS gets off beacon one white line
                m4.setPower(-.7); //Sets motor 4 to power -.2 to move left so OBS gets off beacon one white line
                break; //Exits switch statement
            ...
            case NEXT_BEACON_STEP_TWO:
                ...
                if (odsSensor1.getRawLightDetected() > .3) { //Moves left until next white line is found
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
                ...
                if (gyro.getHeading() > 90) { //If gyro senses a tilt, it lowers the speed of motor 3 and motor 4 to correct itself
                    m1.setPower(-.3); //Sets motor 1 to power -.2 to go left and look for the beacon two white line
                    m2.setPower(.3); //Sets motor 2 to power -2 to go left and look for the beacon two white line
                    m3.setPower(.2); //Sets motor 3 to power .15 to go left and look for the beacon two white line
                    m4.setPower(-.2); //Sets motor 4 to power -.15 to go left and look for the beacon two white line
                    break; //Exits switch statement
                } //End of if statement
                if (gyro.getHeading() < 90) { //If gyro senses a tilt, it lowers the speed of motor 1 and motor 2 to correct itself
                    m1.setPower(-.2); //Sets motor 1 to power -.15 to go left and look for the beacon two white line
                    m2.setPower(.2); //Sets motor 2 to power .15 to go left and look for the beacon two white line
                    m3.setPower(.3); //Sets motor 3 to power .2 to go left and look for the beacon two white line
                    m4.setPower(-.3); //Sets motor 4 to power -.2 to go left and look for the beacon two white line
                    break; //Exits switch statement
                } //End of if statement
                if (gyro.getHeading() == 90) { //If gyro senses no tilt, it continues to go left with all drive train motors set to a power of .2
                    m1.setPower(-.3); //Sets motor 1 to power -.2 to go left and look for the beacon two white line
                    m2.setPower(.3); //Sets motor 2 to power .2 to go left and look for the beacon two white line
                    m3.setPower(.3); //Sets motor 3 to power .2 to go left and look for the beacon two white line
                    m4.setPower(-.3); //Sets motor 4 to power -.2 to go left and look for the beacon two white line
            ...
            case DRIVE_OFF_WHITE_LINE_BEACON_TWO:
                ...
                if (runtime.seconds() > .8) { //Moves right in order to prepare for scan of colors for .8 seconds
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
                ...
                m1.setPower(.5); //Sets motor 1 to power .3 to move right to prepare for color scan
                m2.setPower(-.5); //Sets motor 2 to power -.3 to move right to prepare for color scan
                m3.setPower(-.5); //Sets motor 3 to power -.3 to move right to prepare for color scan
                m4.setPower(.5); //Sets motor 4 to power -.3 to move right to prepare for color scan
                break; //Exits switch statement
            ...
            case BACK_OFF_BEACON_TWO:
                ...
                if (runtime.seconds() > .7) { //Back away from beacon
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
            ...
            case HIT_CAP_BALL:
                ...
                if (gyro.getHeading() > 90) { //If gyro senses a tilt, it lowers the speed of motor 2 to correct itself
                    m1.setPower(0); //Sets motor 1 to power 0 to go backwards diagonally toward the center vortex and cap ball
                    m2.setPower(-.5); //Sets motor 2 to power -.5 to go backwards diagonally toward the center vortex and cap ball
                    m3.setPower(-1); //Sets motor 3 to power -1 to go backwards diagonally toward the center vortex and cap ball
                    m4.setPower(0); //Sets motor 4 to power 0 to go backwards diagonally toward the center vortex and cap ball
                    break; //Exits switch statement
                } //End of if statement
                if (gyro.getHeading() < 90) { //If gyro senses a tilt, it lowers the speed of motor 3 to correct itself
                    m1.setPower(0); //Sets motor 1 to power 0 to go backwards diagonally toward the center vortex and cap ball
                    m2.setPower(-1); //Sets motor 2 to power -1 to go backwards diagonally toward the center vortex and cap ball
                    m3.setPower(-.5); //Sets motor 3 to power -.5 to go backwards diagonally toward the center vortex and cap ball
                    m4.setPower(0); //Sets motor 4 to power 0 to go backwards diagonally toward the center vortex and cap ball
                    break; //Exits switch statement
                } //End of if statement
                if (gyro.getHeading() == 90) { //If gyro senses no tilt, it will continue to move at full power with both motor 2 and 3
            ...
            case UNEXPECTED_NO_COLOR_BEACON_TWO: //Backup incase the correct color isn't found on beacon two within 3 seconds
                ...
                if (runtime.seconds() < .2) { //Move forward (toward beacon) for .2 seconds
                    ...
                    break; //Exits switch statement
                } else if (runtime.seconds() > .2 && colorSensor.blue() > 1.5) { //Moves left until blue is found
                    m1.setPower(0); //Sets motor 1 to power 0 before next step
                    m2.setPower(0); //Sets motor 2 to power 0 before next step
                    m3.setPower(0); //Sets motor 3 to power 0 before next step
                    m4.setPower(0); //Sets motor 4 to power 0 before next step
                    ...
                    break; //Exits switch statement
                } else if (gyro.getHeading() > 300) { //If gyro senses a tilt, it lowers the speed of motor 3 and motor 4 to correct itself
                ...
                else if (gyro.getHeading() > 90) { //If gyro senses a tilt, it lowers the speed of motor 1 and motor 2 to correct itself
                ...
                else if (gyro.getHeading() == 90) { //If gyro senses no tilt, it continues to go right with all drive train motors set to a power of .3
