...
@Autonomous(name = "BLUE", group = "AutoFast")
public class tbBlueSTWILL extends LinearOpMode { // tb = TestBed
    DcMotor m1;
    DcMotor m2;
    DcMotor m3;
    DcMotor m4;
...
//
        runtime.reset();
        while (gyro.getIntegratedZValue() < 25) {
            m1.setPower(-.1); //Turn CC 25 degrees
            m2.setPower(.1);
            m3.setPower(-.1);
            m4.setPower(.1);
            idle();
        ...
        while (runtime.seconds() < .1) {
            m1.setPower(0); //SAhooters ramp all the way up
            m2.setPower(0);
            m3.setPower(0);
            m4.setPower(0);
            m5.setPower(-1);
        ...
        runtime.reset();
        while (gyro.getIntegratedZValue() > -80) {
            m1.setPower(.1);
            m2.setPower(-.1);
            m3.setPower(.1);
            m4.setPower(-.1);
        ...
        runtime.reset();
        while (runtime.seconds() < .1) {
            m5.setPower(0);        //Shooter Stop
            m6.setPower(0);
            idle();
        }

        runtime.reset(); //Back to start
        while (runtime.seconds() < 0.5) {
            m1.setPower(.3); //forward
            m2.setPower(-.3);
            m3.setPower(-.3);
            m4.setPower(.3);
            idle();
        ...
        while (odsSensor1.getRawLightDetected() < .3) {
            if (gyro.getHeading() != 0 && gyro.getIntegratedZValue() < -90) {
                m1.setPower(0); //0.2
                m2.setPower(0.5);
                m3.setPower(1);
                m4.setPower(0); //-0.2
                idle();
            } else if (gyro.getIntegratedZValue() > -90) { //heading >180
                m1.setPower(0); //0.2
                m2.setPower(0.5);
                m3.setPower(1);
                m4.setPower(0); //-0.2
        ...
        while (colorSensor.blue() < 3) {
            if (gyro.getHeading() != 0 && gyro.getIntegratedZValue() < -90) { //heading < 180
                m1.setPower(-.2);
                m2.setPower(.2);
                m3.setPower(.3);
                m4.setPower(-.3);
        ...
        runtime.reset(); //Slight left to get ODS sensor off white line
        ...
        while (odsSensor1.getRawLightDetected() < .3) {
            if (gyro.getIntegratedZValue() != 0 && gyro.getIntegratedZValue() < -90) {//170 {
                m1.setPower(-.3);
                m2.setPower(.3);
                m3.setPower(.6); // .6
                m4.setPower(-.6); // .6
                idle();
            } else if (gyro.getIntegratedZValue() > -90) { //160{
                m1.setPower(-.6); // .6
                m2.setPower(.6); /// .6
                m3.setPower(.3);
                m4.setPower(-.3);
        ...
        while (runtime.seconds() < 2) {
            if (gyro.getHeading() != 0 && gyro.getIntegratedZValue() < -90) { //heading < 180
                ...
                m4.setPower(0);
                idle();
            } else if (gyro.getIntegratedZValue() > -90) { // > 180
                m1.setPower(0);
                m2.setPower(-0.5);
                m3.setPower(-1);
                m4.setPower(0);
